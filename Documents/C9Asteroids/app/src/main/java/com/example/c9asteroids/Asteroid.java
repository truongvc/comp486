...
// Then call the parent setVertices()
    ...
    public void update(float fps){
        setxVelocity ((float) (getSpeed() * Math.cos(Math.toRadians
                (getTravellingAngle() + 90))));
        setyVelocity ((float) (getSpeed() *
                Math.sin(Math.toRadians(getTravellingAngle() + 90))));
        move(fps);
...
// left below 0
        ...
        //we add on an extra point that we wont use in the asteroidVertices[].
